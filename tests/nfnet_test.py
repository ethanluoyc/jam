...
class NFNetTest(absltest.TestCase):
    def test_nfnet(self):
        # # Run NFNet-F0 to classify a single image.
        # You can either run it in eager mode by not jitting the forward function, which
        # will be relatively slow but not incur any compilation cost, or JIT the forward
        # function, which will incur a compilation cost but yield faster inference.

        # If you JIT (the default) then the first time you call the function will
        # trigger the compilation, and all subsequent evaluations will be fast.

        # Load F0 weights
        variant = "F0"
        with open(f"data/checkpoints/nfnets/{variant}_haiku.npz", "rb") as in_file:
        ...
        x = (np.float32(im) - MEAN_RGB) / STDDEV_RGB  # type: ignore
        ...
        self.assertEqual(which_class, "bell pepper")
