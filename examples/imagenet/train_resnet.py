...
def create_model(*, model_cls, half_precision, **kwargs):
    bn_config = {"axis_name": "batch"}
    return model_cls(num_classes=NUM_CLASSES, bn_config=bn_config, **kwargs)
...
def initialized(key, image_size, model):
    ...
    @jax.jit
    def init(*args):
        return model.init(*args, use_running_average=True)
...
def eval_step(state, batch):
    ...
    logits = state.apply_fn(variables, batch["image"], use_running_average=True, mutable=False)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str) -> TrainState:
    ...
    model_cls = getattr(resnet, config.model)
